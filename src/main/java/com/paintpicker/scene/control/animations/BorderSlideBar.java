package com.paintpicker.scene.control.animations;
 ...
 */
public final class BorderSlideBar extends VBox {
     ...
     * @param window
     */
    public BorderSlideBar(Window window, double expandedSize, final ToggleButton controlButton, Pos location, Node... nodes) {
        ...
        controlButton.setOnAction((ActionEvent actionEvent) -> {
            ...
            hidePanel.onFinishedProperty().set((EventHandler<ActionEvent>)
            ...
            showPanel.onFinishedProperty().set((EventHandler<ActionEvent>) 
                    (ActionEvent actionEvent1) -> {
            });
    ...
    private void translateByPos(double size) {
        switch (flapbarLocation) {
            case TOP_LEFT:
                setPrefHeight(size);
//                window.setHeight(window.getHeight() + size);
